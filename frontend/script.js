...
function open_help_modal(table) {
  const modal = document.getElementById('modal');
  const modalTitle = document.getElementById('modal-title');
  const modalForm = document.getElementById('modal-form');
  modalTitle.textContent = 'Help'
  modalForm.innerHTML = generate_help_form(table);
  modal.style.display = 'block';
}

function generate_help_form(table) {
  if (table == 'transactions') {
    return `<h2>新增模式</h2>
    <p>在新增模式中，使用者可以將資料插入到資料庫。所有帶有「<strong>*</strong>」標記的欄位為必填項，未標記的欄位則為選填項。請依據欄位的格式規範，填寫所有必填欄位後，點擊「新增」按鈕，即可成功完成資料新增。</p>

    <h2>查詢模式</h2>
    <p>在查詢模式中，所有欄位均為選填項，使用者可以依需求填寫查詢條件。完成條件設定後，點擊「查詢」按鈕，系統將依據輸入條件篩選出符合的查詢結果。</p>

    <h2>修改模式</h2>
    <p>修改模式分為「條件欄位」和「修改數值欄位」。使用者需先在條件欄位中設定查詢條件，並執行查詢。系統將顯示符合條件的資料列（row），使用者可在修改數值欄位中輸入更新內容，完成後即修改查詢到的資料。</p>

    <h2>刪除模式</h2>
    <p>刪除模式中，條件欄位用於設定刪除條件。系統根據條件搜尋資料，並刪除符合條件的資料列（row）。</p>

    <h2>列印模式</h2>
    <p>列印模式將資料表中的所有資料列（row）以視覺化方式展示在螢幕上，便於檢視。</p>

    <h2>未交運模式（僅適用於交易表格）</h2>
    <p>未交運模式會篩選出搜尋結果中尚未完成交運的資料列（row），並根據金額大小排序。若「折扣後價格」欄位非空（Not Null），則以該欄位進行排序；若該欄位為空，則依「總金額」排序。</p>

    <h2>交運模式（僅適用於交易表格）</h2>
    <p>交運模式會篩選出搜尋結果中已完成交運的資料列（row），並根據金額大小排序。若「折扣後價格」欄位非空（Not Null），則以該欄位進行排序；若該欄位為空，則依「總金額」排序。</p>`
  }
  else {
    return `<h2>新增模式</h2>
    <p>在新增模式中，使用者可以將資料插入到資料庫。所有帶有「<strong>*</strong>」標記的欄位為必填項，未標記的欄位則為選填項。請依據欄位的格式規範，填寫所有必填欄位後，點擊「新增」按鈕，即可成功完成資料新增。</p>
